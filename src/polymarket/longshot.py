...
def loss_correlation_multiscale(rule: LongshotRule, trades: pd.DataFrame,
                                scales=(("day", 86_400.0), ("week", 604_800.0),
                                        ("month", 2_592_000.0),
                                        ("quarter", 7_776_000.0))) -> dict:
    """
    Correlation at several time scales, as a robustness check on the day number.

    This was built to close what looked like a hole -- the worry that day-level
    ICC is blind to a slow regime factor, a season in which favourites
    underperform rather than a bad Saturday. **That worry was wrong, and the
    check that was supposed to fix it is what disproved it.**

    A monthly factor raises BETWEEN-day variance, because every day inside a
    good month shifts together relative to the grand mean. So day-level ICC
    tracks a slow factor rather than missing it. Measured on tapes with a known
    monthly bias injected:

        month bias +/-0.00  ->  day ICC 0.009
        month bias +/-0.02  ->  day ICC 0.014
        month bias +/-0.05  ->  day ICC 0.032
        month bias +/-0.10  ->  day ICC 0.090
        month bias +/-0.30  ->  day ICC 0.285

    Monotone, and close to the month-scale value at every level. A measured day
    ICC of 0.018 therefore already bounds the slow factor: anything as large as
    a +/-0.05 monthly swing would have shown up as roughly 0.032.

    What this function still earns its place for is agreement. If the scales
    disagree, something is wrong with the grouping -- most likely `resolved_at`
    not meaning what it should -- and that is worth knowing before trusting any
    single number.
    """
    out, series = {}, []
    for name, secs in scales:
        r = loss_correlation(rule, trades, bucket_seconds=secs)
        out[name] = r
        if isinstance(r.get("icc"), float):
            series.append((name, r["icc"], r.get("n_buckets", 0)))

    usable = [(n, v, k) for n, v, k in series if k >= 8]
    if len(usable) < 2:
        out["trend"] = {"verdict": "too few usable scales to read a trend"}
        return out

    day = dict((n, v) for n, v, _ in usable).get("day")
    worst = max(v for _, v, _ in usable)
    worst_name = [n for n, v, _ in usable if v == worst][0]
    # Disagreement between scales is the signal now, not "rising". The scales
    # measure the same thing and should roughly agree; if one is far above the
    # others AND above the ruin bar, distrust the grouping before the strategy.
    rising = worst > (day or 0) * 2.0 and worst > 0.07
    out["trend"] = {
        "by_scale": {n: round(v, 4) for n, v, _ in usable},
        "max_icc": round(worst, 4),
        "max_at_scale": worst_name,
        "ruin_threshold_icc": 0.07,
        "scales_disagree": bool(rising),
        "verdict": (f"scales DISAGREE: ICC reaches {worst:.3f} at the "
                    f"{worst_name} scale against {day:.3f} at day scale, and is "
                    f"above the 0.07 bar. Distrust the grouping -- check that "
                    f"resolved_at means what it should -- before trusting "
                    f"either number."
                    if rising else
                    f"scales agree: ICC at or below {worst:.3f} everywhere, "
                    f"under the 0.07 threshold. Bets are close to independent "
                    f"at every horizon tested."),
    }
    return out
